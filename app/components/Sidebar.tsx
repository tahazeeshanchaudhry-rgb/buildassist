  ...
  return (
    <>
      <aside className="hidden w-64 shrink-0 flex-col border-r border-slate-200 bg-white px-4 py-6 md:flex">
        ...
        <div className="mt-auto rounded-2xl bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Workspace</p><p className="mt-2 text-sm font-medium text-slate-700">Your projects</p><p className="mt-1 text-xs text-slate-500">Documents and construction knowledge</p><button type="button" onClick={handleLogout} disabled={isLoggingOut} className="mt-4 text-xs font-semibold text-slate-500 transition hover:text-slate-900 disabled:opacity-50">{isLoggingOut ? "Signing out..." : "Log out"}</button></div>
      </aside>
